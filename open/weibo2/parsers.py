...
class ModelParser(JSONParser):
    ...
    def parse(self, method, payload):
        ...
        if method.payload_list:
            if method.payload_list_key == None:
                raise WeibopError("No payload_list_key for API at (%s)" % method.path)
            result = model.parse_list(method.api, json, method.payload_list_key)
        else:
