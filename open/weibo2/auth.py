...
def _http_call(url, method, authorization, **kw):
    ...
    http_url = '%s?%s' % (url, params) if (method==_HTTP_GET and len(kw) > 0) else url
    ...
    try:
        ...
        logging.error("failed to access (%s), headers:(%s): %s" % (http_url, req.headers, err))
        return None
    body = resp.read()
...
class OAuth2(object):
    ...
    '''
    def __init__(self, app_key = '593038946', app_secret = '56b2959b83a094211ce786472e220165', redirect_uri=None, response_type='code', domain='api.weibo.com', version='2'):
    ...
    def apply_auth(self, headers):
        if self.is_expired():
            raise APIError('21327', 'expired_token', attr)
        headers.update({'Authorization': 'OAuth2 %s' % self.access_token})
