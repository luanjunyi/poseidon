...
#        )(self, *args, post_data=post_data, headers=headers)
    ...
    def me(self):
        """
        This is a dirty fix for sina's inconsistent bug. The screen name is inconsistent
        in several place so this method is broken without this fix.
        ...
        """
        my_tweet = self.user_timeline()
        if len(my_tweet) == 0:
            return self.get_user(screen_name=self.auth.get_username())
        else:
            return my_tweet[0].user
